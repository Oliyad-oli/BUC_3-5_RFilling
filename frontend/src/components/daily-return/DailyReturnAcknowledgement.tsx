...
export function DailyReturnAcknowledgement({
  open,
  onClose,
  ret,
}: {
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4 fade-in" onClick={onClose}>
      <div className="bg-card border border-border rounded-lg max-w-md w-full shadow-lg" onClick={(e) => e.stopPropagation()}>
        <div className="p-6 text-center">
          ...
          <h2 className="text-lg font-semibold">Daily Return Acknowledged</h2>
          <p className="text-sm text-muted-foreground mt-1">Your daily return has been received and validated.</p>
        ...
        <div className="p-4 border-t border-border flex justify-end gap-2">
          <Link to="/returns/daily" className="btn-ghost" onClick={onClose}>View All</Link>
          <Link to="/returns/daily/$id" params={{ id: ret.id }} className="btn-primary" onClick={onClose}>Open Return</Link>
        </div>
      </div>
    </div>
  );
